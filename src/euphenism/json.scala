...
object Json extends Dynamic {
  ...
  object Serializer extends Serializer_1 {
    ...
    implicit def collection[Coll[T1] <: Iterable[T1], T: Serializer]
  ...
  object Deserializer extends Deserializer_1 {
    ...
    implicit def collection[Coll[T1] <: Iterable[T1], T: Deserializer]
  ...
  implicit def option[T: Deserializer]: Deserializer[Option[T]] = new Deserializer[Option[T]]{
    def deserialize(json: JValue): Option[Option[T]] = json match {
        case JNull => Some(None)
        case value: JValue => implicitly[Deserializer[T]].deserialize(value).map(Some(_))
    }
  } 
