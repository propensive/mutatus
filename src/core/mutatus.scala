...
object `package` {
  ...
  implicit class DataBatchExt[T <: Product](values: Iterable[T]) {
    ...
    def saveAll()(implicit svc: Service,
      ...
      val (batch, refs) =
        values.foldLeft(svc.readWrite.newBatch() -> mutable.Map.empty[T, Ref[T]]) {
      ...
      batch.submit()
      refs.toMap //batch may supply only those keys which were generated by Datastore, but in mutatus generates keys deterministically based on entity content
    }
      ...
      encoder.encode(value) match {
        case entityValue: EntityValue =>
          Entity
            ...
            .build()
        case _ => throw MutatusException("Unable to store non entity type")
      }
...
object Decoder extends Decoder_1 {
  ...
  def combine[T](caseClass: CaseClass[Decoder, T]): Decoder[T] = {
            ...
            param.default match {
              ...
              case None =>
                throw MutatusException(
                  s"Failed to generate decoder for ${caseClass.typeName.full}.${param.label}: ${ex.getMessage}, EntityKeys: ${entity.getNames.asScala
                    .mkString(", ")}"
                )
            }
        }.get
    ...
    case _ =>
      throw MutatusException(
        "Tried to decode case class from value which is not EntityValue"
      )
  }
