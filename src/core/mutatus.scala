...
object `package` {
  ...
  implicit class DataBatchExt[T <: Product](values: Iterable[T]) {
    ...
    def saveAll()(implicit svc: Service, encoder: Encoder[T], dao: Dao[T], idField: IdField[T]): Map[T, Ref[T]] = {
      val (batch, refs) = values.foldLeft(svc.readWrite.newBatch() -> mutable.Map.empty[T, Ref[T]]) {
    ...
    def deleteAll()(implicit svc: Service, dao: Dao[T], idField: IdField[T]): Unit = {
      val _ = values.foldLeft(svc.readWrite.newBatch()) {
        case (b, value) => b.delete(idField.idKey(idField.key(value)).newKey(dao.keyFactory))
          b
      }.submit()
    }
  }
  ...
  implicit class DataExt[T <: Product](value: T) {
    ...
    private[mutatus] def buildEntity()(implicit encoder: Encoder[T], dao: Dao[T], idField: IdField[T]): Entity = encoder.encode(value) match {
    ...
    def save()(implicit svc: Service, encoder: Encoder[T], dao: Dao[T], idField: IdField[T]): Ref[T] = new Ref[T](
