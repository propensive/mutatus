...
case class MutatusE2ESpec()(implicit runner: Runner) {
  ...
  def generateComplex[Id](idFn: Int => Id)(idx: Int): TestComplex[Id] = {
    val listOptInt = 0.until(idx).map(Some.apply).filter(_.exists(_ % 2 == 0)).toList
    val listOptInner = 0
      .until(idx)
      .toList
      .map(
...

    {
      idFn(idx) match {
        case id: String =>
          TestComplexStringId(id,
                              listOptInt,
                              listOptInner.lastOption.flatten,
                              listOptInner)
        case id: Long =>
          TestComplexLongId(id, listOptInt, listOptInner.lastOption.flatten, listOptInner)
        case id: Guid =>
          TestComplexGuid(id, listOptInt, listOptInner.lastOption.flatten, listOptInner)
      }
  ...
  stringIdComplexEntities.foreach { e =>
    test("save entities - complex - string id")(e.save().ref.getName()).assert(_ == e.id)
  }

  guidComplexEntities.foreach { e =>
    test("save entities - complex - guid")(e.save().ref.getName()).assert(_ == e.id.guid)
  }

  test("save entites in batch mode")(batchedSimpleEntities.saveAll()).assert { result =>
    result.nonEmpty && result.forall {
      case (ent, ref) => ref.ref.getId() == ent.id
    }
  }

  test("fetch entities - simple")(Dao[TestSimpleEntity].all().toVector.sortBy(_.id))
  ...
  guidComplexEntities
    ...
    .foreach(
      e =>
        test("fetch entity by id - complex - guild")(Dao[TestComplexGuid].unapply(e.id))
          .assert(_.contains(e))
    )

//   it should "query entities" in {

//   }

  simpleEntities.take(5).foreach { entity =>
    val updated = entity.copy(decimalParam = entity.decimalParam * 10.0,
...
object MutatusE2ESpec {
  object Model {
    sealed trait SomeState
    object SomeState {
      def apply(idx: Int): SomeState =
        if (idx <= 2) OkButWithQuestionMark
        else if (idx > 6) Ok
        else MarkedAs(idx)

      case object Ok extends SomeState
      case object OkButWithQuestionMark extends SomeState
      case class MarkedAs(mark: Int) extends SomeState
    }
    case class TestSimpleEntity(@id id: Int,
                                longParam: Long,
                                decimalParam: Double,
                                stringParam: String,
