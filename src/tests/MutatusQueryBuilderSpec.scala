...
case class MutatusQueryBuilderSpec()(implicit runner: Runner) {
  ...
  List(
    ...
    builder.where(_.innerClass.intParam == 1) -> PropertyFilter.eq("innerClass.intParam",
                                                                   1),
    ...
    builder.where(_.innerClassOpt.exists(_.deeper.inner.some3 == 3)) -> PropertyFilter.eq(
      "innerClassOpt.deeper.inner.some3",
      3
    ),
    builder.where(_.innerClassOpt.exists(_.intParam == 1)) -> PropertyFilter.eq(
      "innerClassOpt.intParam",
    ...
    builder.where(_.innerClassOpt.map(_.decimalParam).contains(2.0)) -> PropertyFilter.eq(
      "innerClassOpt.decimalParam",
      2.0
    ),
    builder.where(
  ...
  List(
    ...
    case ((query: QueryBuilder[QueringTestEntity], expected), idx) =>
      test("build orderBy criteria")(query.orderCriteria).assert(orderCriteria => {
